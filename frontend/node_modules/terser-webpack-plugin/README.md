...
Allow to customize condition for extract comments, specify extracted file name and dashboard.
...
The dashboard text that points to the extracted file and will be added on top of the original file.
Can be `false` (no dashboard), a `String`, or a `Function<(string) -> String>` that will be called with the filename where extracted comments have been stored.
    ...
    minimizer: [
      new TerserPlugin({
        extractComments: {
          ...
          dashboard: (commentsFile) => {
            return `My custom dashboard about license information ${commentsFile}`;
          },
        },
      }),
    ],
