...
setTimeout(() => {
  // After a short delay the configuration is changed and a dashboard plugin is added
  // to the config
  compiler.apply(new webpack.BannerPlugin('A new dashboard'));

  // Recompile the bundle with the dashboard plugin:
  instance.invalidate();
}, 1000);
```
